...
export default class IScrollView extends cc.ScrollView {
    ...
    /** 是否改变了item */
    private isChangeItem: boolean = false;
    ...
    setItemPrefab(prefab: cc.Prefab) {
    ...
    refreshDataProvider() {
        ...
        if (this.dataProvider.length < items.length) {
            const diff = items.length - this.dataProvider.length;
            for (let i = 0; i < items.length && i < diff; i++) {
                items[items.length - 1 - i].active = false;
            }
        }
        ...
        if (this.dataProvider) {
            if (this.horizontal) {
                ...
                let height = (this.content.height =
                    this.dataProvider.length * (this.itemPrefab.data.height + this.spacing) + this.spacing);
