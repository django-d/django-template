...
export default class VMState extends VMBase {
    ...
    public onValueInit() {
        ...
        console.log('初始化 state: ' + this.watchPath + ' value:' + value);
    ...
    private checkNodeFromValue(value) {
        this.watchNodes.forEach((node, index) => {
            let v = '';
            switch (this.foreachChildType) {
                case CHILD_MODE_TYPE.CUSTOM_VALUE:
                    v = this.conditionValue;
                    break;
                case CHILD_MODE_TYPE.NODE_INDEX:
                    v = index + '';
                    break;
                case CHILD_MODE_TYPE.NODE_NAME:
                    v = node.name;
                    break;
            }
    ...
    /**条件检查 */
    private conditionCheck(value, a, b?): boolean {
        let cod = CONDITION;
        let v = `${value}`;
        switch (this.condition) {
            ...
            case cod['!=']:
                ...
                break;
            case cod['<']:
                if (Number(v) < Number(a)) return true;
                break;
            case cod['>']:
                console.log('Number(v) :', Number(v), 'Number(v):', Number(a));
                if (Number(v) > Number(a)) return true;
                break;
            case cod['>=']:
                if (Number(v) >= Number(a)) return true;
