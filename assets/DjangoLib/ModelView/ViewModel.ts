...
import { iVMEvent } from '../Descript/eventDecorator';

const VM_EMIT_HEAD = 'VC:';
const DEBUG_SHOW_PATH = CC_DEBUG;
...
class ViewModel<T> {
    ...
    private _callback(n: any, o: any, path: string[]): void {
        if (this.active == true) {
            ...
            if (DEBUG_SHOW_PATH) cc.log('>>', n, o, name);
            iVMEvent.emit(name, n, o, [this._tag].concat(path)); //通知末端路径
            if (this.emitToRootPath) iVMEvent.emit(VM_EMIT_HEAD + this._tag, n, o, path); //通知主路径
...
export class VMManager {
    ...
    bindPath(path: string, callback: Function, target?: any, useCapture?: boolean): void {
        ...
        iVMEvent.on(VM_EMIT_HEAD + path, callback as any);
    }
    ...
    unbindPath(path: string, callback: Function, target?: any): void {
        ...
        iVMEvent.removeListener(VM_EMIT_HEAD + path, callback as any);
    }
